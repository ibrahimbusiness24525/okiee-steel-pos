...
function BottomNav({ user, active, setActive, onLogout }) {
  ...
  const adminNav = [
    ...
    {key:"expenses",     label:t.expenses || (isUrdu ? "اخراجات" : "Expenses"), icon:ICONS.wallet},
    {key:"ledger",       label:t.ledger,    icon:ICONS.book},
    {key:"records",      label:t.records || (isUrdu ? "ریکارڈز" : "Records"), icon:ICONS.invoice},
...
function Topbar({ active, user, isMobile }) {
  ...
  const titles = {
    ...
    expenses:t.expensesManagement || t.expenses,
    ledger:t.ledgerManagement,
    records:t.recordsManagement || t.records,
